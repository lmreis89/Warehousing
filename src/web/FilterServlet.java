...
public class FilterServlet extends HttpServlet
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		{
			if(occur==1)
				proc.addFactFilterAfter(factID, op, value); 
			else
				proc.addFactFilterBefore(factID, op, value);
		}
		else
		{
			if(occur==1)
				proc.removeFactFilterAfter(factID); 
			else
				proc.removeFactFilterBefore(factID);
		}
